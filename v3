...
Changes:
  ...
  4. All existing fixes retained (NaN sanitisation, session fallback, etc.)
"""
...
async def run_variance(request: Request):
    ...
    # Rename top5/bot5 for display
    top5_display = _rename_cols(_sanitise_df(top5), scenario_a, scenario_b)
    bot5_display = _rename_cols(_sanitise_df(bot5), scenario_a, scenario_b)

    # Build hotspot using internal names, then rename inside
    hotspot_raw = _build_hotspot(leaf_df, group_fields, favorable_is_lower)
    hotspot = [_rename_record_keys(r, scenario_a, scenario_b) for r in hotspot_raw]
    ...
    return JSONResponse({
        ...
        "group_fields":       group_fields,
        "top5":               _safe_records(top5_display),
        "bot5":               _safe_records(bot5_display),
        "hotspot":            hotspot,
    })
...
def _build_hotspot(leaf_df: pd.DataFrame, group_fields: list, fav_lower: bool) -> list:
    """Returns records with internal column names (A, B, delta, pct). Caller renames."""
