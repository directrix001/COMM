  ...
  function buildHTML() {
    return `
      ...
      <style>
        ...
        /* ── Header ──────────────────────────────────────────────────── */
        .va-tree-table th {
          background: #1a1a2e;       /* deep navy — strong contrast anchor */
          color: #f0f4ff;
          font-weight: 600;
          font-size: .75rem;
          letter-spacing: .03em;
        ...
        /* ── Level 0 — top group (lightest blue tint) ────────────────── */
        .va-tree-row[data-level="0"] {
          background: #eef2fb;
        }
        .va-tree-row[data-level="0"] td {
          color: #0f1f4b;
        }
        .va-tree-row[data-level="0"] td.tree-label { font-weight: 700; }

        /* ── Level 1 — sub-group (very light grey-blue) ──────────────── */
        .va-tree-row[data-level="1"] {
          background: #f5f7fc;
        }
        .va-tree-row[data-level="1"] td {
          color: #1a2a52;
        }
        .va-tree-row[data-level="1"] td.tree-label { font-weight: 600; }

        /* ── Level 2+ — leaf rows (pure white) ───────────────────────── */
        .va-tree-row.leaf-row {
          background: #ffffff;
        }
        .va-tree-row.leaf-row td { color: #2c3e6b; }

        /* zebra stripe on leaves only for readability */
        .va-tree-row.leaf-row:nth-child(even) { background: #fafbfe; }

        /* subtotal rows re-use level backgrounds; just enforce bold */
        .va-tree-row.subtotal-row td { font-weight: 600; }

        .va-tree-row.hidden { display: none; }

        /* hover across all body rows */
        ...
        /* ── Variance coloring (readable on light bg) ────────────────── */
        .var-fav  { color: #16803c; font-weight: 600; }   /* deep green  */
        .var-adv  { color: #b91c1c; font-weight: 600; }   /* deep red    */
        .var-zero { color: #64748b; }                      /* slate-grey  */
      </style>
    `;
  }
