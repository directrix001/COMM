...
(function () {
  ...
  function bindSrcMode() {
    ...
    // Load Tab1 button — calls dedicated POST endpoint that reads final_db_bytes
    document.getElementById('t2-btn-load-tab1').addEventListener('click', async () => {
      ...
      try {
        // POST /api/tab2/load-tab1 promotes Tab 1 session data → master_db_bytes
        // then returns filter options exactly like the upload endpoints do
