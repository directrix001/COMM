 ...
 *  3. Variance Hotspot section removed
 *  4. Top/Bottom movers now driven by a slider (default 10, max 20)
 *     with Variance % shown alongside values
 *
 * Flow:
 ...
 *  4. "Run Variance" → POST /api/tab2/run → render metrics, pivot table, top/bottom movers
...
(function () {
    ...
    return `
      ...
      <div id="t2-results" style="display:none">
        ...
        <div class="d-flex align-items-center justify-content-between mb-2 flex-wrap gap-2">
          <div class="va-section-label mb-0"><i class="bi bi-bar-chart-steps"></i> Top &amp; Bottom Movers</div>

          <!-- Slider control -->
          <div class="d-flex align-items-center gap-3" style="min-width:260px">
            <label style="font-size:.75rem;font-weight:600;color:var(--text-muted);white-space:nowrap">
              Show top
            </label>
            <input
              type="range"
              ...
              style="flex:1;accent-color:#4a6cf7;cursor:pointer"
            />
            <span
              id="t2-movers-count"
              style="
        ...
        <div class="row g-3">
          <div class="col-md-6">
            <p class="mb-1" style="font-size:.78rem;font-weight:600" id="t2-top5-label">Top 10 Favourable</p>
            <div class="va-table-wrap"><div class="va-table-scroll" id="t2-top5-wrap"></div></div>
          </div>
          <div class="col-md-6">
            <p class="mb-1" style="font-size:.78rem;font-weight:600" id="t2-bot5-label">Top 10 Adverse</p>
      ...
      <style>
        ...
        #t2-movers-slider::-webkit-slider-thumb {
          width: 16px;
          height: 16px;
        }
        ...
        .va-mini-table th:last-child { border-right: none; }
        .va-mini-table th.num { text-align: right; }
        .va-mini-table td {
          padding: 5px 10px;
          border-bottom: 1px solid #e4e9f0;
          border-right: 1px solid #edf0f7;
        ...
        .va-mini-table td:last-child { border-right: none; }
        .va-mini-table td.num {
          text-align: right;
          font-variant-numeric: tabular-nums;
        }
        .va-mini-table tr:nth-child(even) td { background: #fafbfe; }
        .va-mini-table tr:hover td { background: #dde8f8 !important; }
        .va-mini-table tr.rank-row td:first-child {
          font-weight: 700;
          color: #4a6cf7;
        }
      </style>
  ...
  /* ── MOVERS SLIDER ──────────────────────────────────────────────────── */
  function bindMoversSlider() {
    // Slider is inside #t2-results which is built on init but hidden;
    // we use event delegation via document since the element exists in DOM after init().
    document.addEventListener('input', e => {
      if (e.target && e.target.id === 't2-movers-slider') {
        const n = parseInt(e.target.value, 10);
        const countEl = document.getElementById('t2-movers-count');
        if (countEl) countEl.textContent = n;
        if (lastResult) {
          _rerenderMovers(lastResult, n,
            document.getElementById('t2-fav-mode') &&
            document.getElementById('t2-fav-mode').value === 'lower');
        }
      }
    });
  }

  /** Get current slider value (falls back to 10) */
  function _getMoversN() {
    const el = document.getElementById('t2-movers-slider');
  ...
  function buildTreeTable(treeRecords, colA, colB, favLower, groupFields) {
    ...
    dimCols.forEach((f, i) => {
      ...
      if (i === 0) th.style.minWidth = '140px';
      hrow.appendChild(th);
    });
    ...
    rows.forEach(row => {
      ...
      dimCols.forEach((_, colIdx) => {
        ...
        if (colIdx === row.level) {
          td.style.paddingLeft = `${8 + row.level * 4}px`;
          ...
          txt.textContent = row.dimValues[colIdx] || '(blank)';
  ...
  /* ══════════════════════════════════════════════════════════════════════
   * MOVERS TABLE — rich mini table with rank, value, and percentage
   * ══════════════════════════════════════════════════════════════════════ */

  /**
   * Build a styled mini table for top/bottom movers.
   * Columns: Rank | <dim fields> | Scenario A | Scenario B | Variance | Variance %
   * @param {Array}   records   – full list from backend (up to 20)
   * @param {number}  n         – how many rows to show (slider value)
   * @param {string}  colA
   * @param {string}  colB
   * @param {boolean} favLower
   * @param {Array}   dimCols   – dimension column names to show (e.g. group_fields)
   */
  function buildMoversTable(records, n, colA, colB, favLower, dimCols) {
    ...
    rankTh.textContent = '#';
    rankTh.style.width = '32px';
    hrow.appendChild(rankTh);

    // Show dimension columns that exist in the records
    const recordKeys = Object.keys(sliced[0] || {});
    const shownDims = dimCols.filter(d => recordKeys.includes(d));
    // Fallback: if no dims found in records, try to pick non-numeric keys
    const fallbackDims = shownDims.length
      ? shownDims
      : recordKeys.filter(k => ![colA, colB, 'Variance', 'Variance %'].includes(k));

    fallbackDims.forEach(d => {
      const th = document.createElement('th');
      th.textContent = d;
      hrow.appendChild(th);
    });
    ...
    sliced.forEach((rec, idx) => {
      ...
      rankTd.style.color = '#4a6cf7';
      rankTd.style.textAlign = 'center';

      // Dimension cells
      fallbackDims.forEach(d => {
        const td = tr.insertCell();
        td.textContent = rec[d] != null ? rec[d] : '–';
        td.style.maxWidth = '180px';
        td.style.overflow = 'hidden';
        td.style.textOverflow = 'ellipsis';
      });

      // Numeric cells: A, B, Variance, Variance %
      const aVal   = rec[colA];
      const bVal   = rec[colB];
      const varVal = rec['Variance'];
      const pctVal = rec['Variance %'];

      [aVal, bVal, varVal, pctVal].forEach((val, ci) => {
        const td = tr.insertCell();
        td.className = 'num';
  ...
  /**
   * Re-render only the movers section (called by slider without re-running analysis).
   */
  function _rerenderMovers(data, n, favLower) {
    const colA     = data.col_a || data.scenario_a || 'Scenario A';
    const colB     = data.col_b || data.scenario_b || 'Scenario B';
    const dimCols  = data.group_fields || [];

    const top5Label = document.getElementById('t2-top5-label');
    const bot5Label = document.getElementById('t2-bot5-label');
    if (top5Label) top5Label.textContent = `Top ${n} ${favLower ? 'Favourable' : 'Positive'}`;
    if (bot5Label) bot5Label.textContent = `Top ${n} ${favLower ? 'Adverse'    : 'Negative'}`;

    const topWrap = document.getElementById('t2-top5-wrap');
    const botWrap = document.getElementById('t2-bot5-wrap');

    if (topWrap) {
      topWrap.innerHTML = '';
      topWrap.appendChild(buildMoversTable(data.top5 || [], n, colA, colB, favLower, dimCols));
    }
    if (botWrap) {
      botWrap.innerHTML = '';
      botWrap.appendChild(buildMoversTable(data.bot5 || [], n, colA, colB, favLower, dimCols));
    }
  }
  ...
  function renderResults(data, favLower) {
    ...
    // ── Top / Bottom N movers ────────────────────────────────────────────
    // Reset slider to default 10 on each fresh run
    const slider  = document.getElementById('t2-movers-slider');
    const countEl = document.getElementById('t2-movers-count');
    if (slider)  slider.value = 10;
    if (countEl) countEl.textContent = '10';

    _rerenderMovers(data, 10, favLower);
